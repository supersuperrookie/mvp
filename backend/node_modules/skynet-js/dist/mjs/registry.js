...
import { validateBigint, validateHexString, validateObject, validateOptionalObject, validateString, } from "./utils/validation";
...
export async function getEntry(publicKey, dataKey, customOptions) {
    ...
    if (signedEntry &&
        !sign.detached.verify(hashRegistryEntry(signedEntry.entry, opts.hashedDataKeyHex), new Uint8Array(signedEntry.signature), hexToUint8Array(publicKey))) {
 ...
 * @throws - Will throw if the given key is not valid.
 */
export async function getEntryLink(publicKey, dataKey, customOptions) {
    validateString("publicKey", publicKey, "parameter");
    validateString("dataKey", dataKey, "parameter");
    validateOptionalObject("customOptions", customOptions, "parameter", defaultGetEntryOptions);
    const opts = {
...
export function validateRegistryEntry(name, value, valueKind) {
    ...
    validateString(`${name}.data`, value.data, `${valueKind} field`);
